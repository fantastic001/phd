...
* We propose a systematic empirical study of how partition count affects partition assignment quality for online vertex arrivals in dynamic graph embedding systems, evaluated against a uniform-random partitioning baseline, together with a preliminary exploration of the replication factor as an additional lever.
* We evaluate the impact of partition count on embedding quality, partition balance, and edge cut, and provide preliminary, single-configuration evidence that increasing the replication factor can recover embedding quality lost to early, uncertain assignment at the cost of additional computation.
* We provide empirical evidence and analysis of this quality/computation trade-off, contributing to the understanding of how partition count, and preliminarily replication, affect partitioning in distributed dynamic graph embedding systems.
...
| 4 | 57.00% | 52.00% | 49.92% |
| 8 | 57.00% | 33.93% | 46.61% |
Table: F1 reconstruction score vs. $\mu$ and partition count for AstroPh (buffer size 1000, $RF=1$).
...
![AstroPh: last-iteration F1 score, and wall-clock partitioning/embedding time, vs. buffer size](png/astroph-buffer-timing.png)
...
|---|---|---|---|---|---|
| AS-Oregon | P=1 vs P=2 | 0.572 | 0.629 | $-0.54$ | No |
| AS-Oregon | P=1 vs P=4 | 0.095 | 0.114 | $-1.93$ | No |
| AS-Oregon | P=1 vs P=8 | 0.019 | 0.057 | $-4.17$ | Yes |
| AS-Oregon | P=2 vs P=4 | 0.796 | 0.700 | $-0.23$ | No |
| AS-Oregon | P=4 vs P=8 | 0.062 | 0.100 | $-2.13$ | No |
| AstroPh | P=1 vs P=2 | $<0.001$ | 0.029 | $-9.91$ | Yes |
| AstroPh | P=1 vs P=4 | $<0.001$ | 0.029 | $-17.86$ | Yes |
| AstroPh | P=1 vs P=8 | $<0.001$ | 0.029 | $-27.70$ | Yes |
| AstroPh | P=2 vs P=4 | $<0.001$ | 0.029 | $-7.89$ | Yes |
| AstroPh | P=4 vs P=8 | $<0.001$ | 0.029 | $-9.02$ | Yes |
| CITESEER | P=1 vs P=2 | 0.006 | 0.005 | $-1.42$ | Yes |
| CITESEER | P=1 vs P=4 | 0.365 | 0.358 | $+0.40$ | No |
| CITESEER | P=1 vs P=8 | $<0.001$ | $<0.001$ | $+4.29$ | Yes |
| CITESEER | P=2 vs P=4 | $<0.001$ | $<0.001$ | $+3.83$ | Yes |
| CITESEER | P=4 vs P=8 | $<0.001$ | $<0.001$ | $+5.24$ | Yes |
| DBLP | P=1 vs P=2 | $<0.001$ | 0.029 | $+10.80$ | Yes |
| DBLP | P=1 vs P=4 | $<0.001$ | 0.029 | $+12.66$ | Yes |
| DBLP | P=1 vs P=8 | 0.005 | 0.029 | $-3.65$ | Yes |
| DBLP | P=2 vs P=4 | 0.029 | 0.057 | $+2.04$ | Yes |
| DBLP | P=4 vs P=8 | $<0.001$ | 0.029 | $-26.00$ | Yes |
| Enron | P=1 vs P=2 | $<0.001$ | 0.100 | $+13.56$ | Yes |
| Enron | P=1 vs P=4 | 0.040 | 0.100 | $+3.80$ | Yes |
| Enron | P=1 vs P=8 | 0.002 | 0.100 | $+12.63$ | Yes |
| Enron | P=2 vs P=4 | 0.323 | 0.100 | $+1.05$ | No |
| Enron | P=4 vs P=8 | 0.118 | 0.200 | $+1.84$ | No |
Table: Pairwise significance tests on the per-run final-iteration reconstruction F1 across partition counts, $RF = 1$. Runs per configuration: CITESEER 11, AstroPh and DBLP 4, AS-Oregon 4 at $P=1$ and 3 at $P \in \{2, 4, 8\}$, Enron 3. A negative Cohen's $d$ means F1 decreases as the partition count grows.

The tests confirm the dataset-dependent picture drawn from the mean scores. On **AstroPh**, every comparison is significant under both tests with very large negative effect sizes ($d$ between $-7.9$ and $-27.7$): F1 degrades monotonically and unambiguously with each doubling of the partition count. On **AS-Oregon**, the degradation is milder and only the $P=1$ vs $P=8$ comparison reaches significance under the $t$-test ($p = 0.019$, $d = -4.2$), with the Mann--Whitney test borderline ($p = 0.057$) given only three runs per configuration; the direction is consistently negative but the individual per-step differences are within noise. On **CITESEER** and **Enron**, the significant comparisons are predominantly *positive*: CITESEER dips slightly from $P=1$ to $P=2$ ($d = -1.4$) and then rises strongly ($d = +5.2$ from $P=4$ to $P=8$), and Enron increases at every step (significant up to $P=4$, $d = +13.6$ for $P=1$ vs $P=2$). **DBLP** is a mixed case rather than a clearly positive one: it rises significantly through $P=4$, but then reverses sharply — the $P=4$ vs $P=8$ comparison ($d=-26.0$) is the single largest-magnitude effect size in the entire table, larger than any AstroPh comparison, and $P=1$ vs $P=8$ is also a significant decrease ($d=-3.65$), so DBLP ends up significantly *below* its own $P=1$ baseline at the largest partition count tested even though its trajectory up to $P=4$ is positive. AstroPh remains the only dataset whose decline is monotone across the *entire* $P=1$–$P=8$ range; DBLP's decline is confined to its last step, but in isolation it is larger than any single AstroPh comparison.


Two caveats apply. The samples are small (as few as three runs per configuration), so the Mann--Whitney test has little power: on Enron it cannot reach $p < 0.05$ even where the $t$-test and a very large $d$ jointly indicate a clear effect. The same floor effect works the other way for AstroPh and DBLP ($n=4$ per group): the two-sided Mann–Whitney $p$-value is bottomed out at $2/\binom{8}{4} \approx 0.029$ for any pair with no overlap, so for these two datasets every "Yes" in the Mann–Whitney column mainly confirms non-overlap rather than adding evidence beyond the $t$-test. In addition, the comparisons are performed without correction for multiple testing; applying a Bonferroni correction across the five comparisons per dataset, or an omnibus test (ANOVA or Kruskal--Wallis) across all partition counts at once, would be the stricter alternative. The effect sizes are reported alongside the $p$-values precisely so that a non-significant result is not misread as evidence of equivalence.
...
The results show that the impact of partitioning on embedding quality is dataset-dependent, though not a clean three-vs-two split. AstroPh degrades monotonically and significantly with every doubling of $P$ (Cohen's $d$ from $-7.9$ to $-27.7$), and AS-Oregon also trends downward, though only its $P=1$ vs $P=8$ gap is statistically distinguishable from run-to-run noise. CITESEER and Enron are stable or improve significantly across most of the partition range. DBLP is the intermediate case: it improves significantly up to $P=4$ but then reverses sharply, ending significantly below its own $P=1$ baseline at $P=8$ with the single largest-magnitude effect size measured anywhere in this study ($d=-26.0$ for $P=4$ vs $P=8$) — partitioning does not help DBLP uniformly, only up to a point. Pairwise Welch $t$-tests and Mann--Whitney $U$ tests on the per-run final F1 scores, together with Cohen's $d$ effect sizes, back this characterization, with the caveat that the very small within-configuration standard deviations observed (e.g. AstroPh $\pm 0.11\%$ at $P=1$) mean that even modest percentage-point differences register as statistically significant with large effect sizes; the percentage-point magnitude, not the $p$-value or $d$ alone, is what should carry the practical interpretation. Of the dataset properties measured (density, clustering coefficient, modularity), graph density correlates most closely with this split, though it does not fully explain it, since no single threshold on density, clustering coefficient, or modularity cleanly separates the two groups; identifying the underlying driver is left to future work. On most datasets, the partitioner keeps balance close to the ideal value, though CITESEER shows a more pronounced imbalance at higher partition counts; across all datasets and partition counts it keeps the repartitioning rate low and decaying after an initial warm-up period, indicating that a simple neighbor-based, buffer-driven partitioning strategy is a practical and largely, though not universally, stable choice for online graph partitioning. Partitioning time itself is consistently smaller than embedding time — directly measured at roughly 20–30% of embedding time at the buffer size used throughout this study, and an even smaller share at lower buffer sizes — confirming that the dominant systems bottleneck in this setting is the embedding computation on the largest or most active partition rather than the assignment decision itself.

The replication factor experiment provides preliminary evidence for the trade-off that motivates this paper: in a single exploratory configuration, hedging a vertex's partition assignment across multiple partitions and averaging the resulting embeddings substantially mitigated the quality loss caused by early, uncertain assignment decisions, at the cost of additional computation proportional to the replication factor. This suggests that systems which must assign vertices to partitions immediately, with little information about their eventual neighborhood, benefit from replication as a way to hedge against a suboptimal initial assignment, while systems that can tolerate additional latency may instead delay assignment until more information is available, avoiding the replication overhead altogether. Together, these findings indicate that neither immediate nor delayed assignment is uniformly preferable; the right choice depends on the graph's community structure and on which resource, latency or computation, is more constrained in a given deployment.
